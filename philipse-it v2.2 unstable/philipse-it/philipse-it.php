<?php
/*
Plugin Name: Philipse IT
Description: Websupport@philipse-it.nl voor meer informatie.
Requires at least: 4.2
Requires PHP: 5.6.40
Version: 2.2
License: GPL
Author: Feike Falkena
Author URI: http://philipse-it.nl
...
function test_plugin_setup_menu(){
	add_menu_page( 'Philipse IT', 'Philipse IT', 'manage_options', 'pit', 'pit_options_page', plugin_dir_url( __FILE__ ) . '/img/icon.png', 3  );
}
?>
<style>
.toplevel_page_pit .wp-menu-image img{
	width: 15px;
	height: 15px;
}

#toplevel_page_pit{
	border: 2px solid #77bd7e;
}

</style>
<?php
/* TOEVEOGEN AAN INSTELLINGEN ALS SUBMENU
...
function pit_settings_init(  ) { 
	...
	add_settings_section(
		...
		__( '<br>Diverse opties:', 'wordpress' ), 
...
/*	add_settings_field( 
	...
	add_settings_field( 
		'pit_checkbox_field_17', 
		__( 'Site Kit', 'Google' ), 
		'pit_checkbox_field_17_render', 
		'pluginPage', 
		'pit_pluginPage_section_2' 
	);
	add_settings_field( 
		'pit_checkbox_field_18', 
		__( 'Login logo', 'Philipse IT' ), 
		'pit_checkbox_field_18_render', 
		'pluginPage', 
		'pit_pluginPage_section_2' 
	);
	add_settings_section(
		'pit_pluginPage_section_3', 
		__( '<br>Google Analytics:', 'wordpress' ), 
		'pit_settings_section_callback', 
		'pluginPage'
	);
	add_settings_field( 
		'pit_checkbox_field_20', 
		__( 'Google Analytics code', 'Philipse IT' ), 
		'pit_checkbox_field_20_render', 
		'pluginPage', 
		'pit_pluginPage_section_3' 
	);
...
function pit_checkbox_field_20_render(  ) { 
	$options = get_option('pit_settings');
	echo "<textarea id='pit_checkbox_field_20' name='pit_settings[text_area]' rows='7' cols='50' type='textarea'>{$options['text_area']}</textarea>";

	$GT = $options['text_area'];
...
<script async src="https://www.googletagmanager.com/gtag/js?id=<?php echo $GT_CODE;?>"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', '<?php echo $GT_CODE;?>');
</script>
...
function pit_checkbox_field_17_render(  ) { 
	$options = get_option( 'pit_settings' );
	$checked = (isset($options['pit_checkbox_field_17'])) ? 'checked' : '';
	?>
	<input type='checkbox' name='pit_settings[pit_checkbox_field_17]' <?php echo $checked; ?> value='1'>
	<?php
}
function pit_checkbox_field_18_render(  ) { 
	$options = get_option( 'pit_settings' );
	$checked = (isset($options['pit_checkbox_field_18'])) ? 'checked' : '';
	?>
	<input type='checkbox' name='pit_settings[pit_checkbox_field_18]' <?php echo $checked; ?> value='1'>
	<?php
}
...
if (isset($options['pit_checkbox_field_17'])) {
	// REMOVE GOOGLE SITE KIT
...
if (isset($options['pit_checkbox_field_18'])) {
	add_action( 'login_enqueue_scripts', 'my_login_logo2' );
}
